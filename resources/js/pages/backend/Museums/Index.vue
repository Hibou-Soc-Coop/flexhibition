...
import museumPlaceholder from '@assets/placeholders/museum.png';
        ...
        <PageLayout title="Elenco Musei">
            <template #button>
                <div class="flex gap-2">
                    <Button
                        v-if="props.museums.length < Number(props.maxMuseum)"
                        @click="router.visit(museumsRoutes.create().url)"
                        colorScheme="create"
                        class="ml-6 h-8"
                        >Aggiungi nuovo museo</Button
                    >
                </div>
            </template>
            <div class="gap-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                <Card
                    ...
                    :route="museumsRoutes"
                    :id="museum.id"
                    :title="getTranslation(museum.name, primaryLanguageCode)"
                    :excerpt="truncate(getTranslation(museum.description, primaryLanguageCode), 60)"
                    :thumbnail="museum.logo.url || museumPlaceholder"
                ></Card>
                <div
                    v-if="props.museums.length === 0"
