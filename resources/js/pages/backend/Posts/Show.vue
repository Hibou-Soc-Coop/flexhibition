...
<template>
    <Head :title="props.post.name[primaryLanguageCode] + ' - Dettaglio'" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <PageLayout title="Informazioni dell'Opera">
            ...
            >
                <Button
                    @click="router.visit(postsRoutes.edit.url(props.post.id))"
                    color-scheme="edit"
                    class="mr-2"
                >
                    Modifica
            ...
            <div class="grid grid-cols-[1fr_4fr] grid-rows-[auto_auto] gap-4">
                <div class="col-start-1 col-end-2 rounded-lg border p-4 shadow">
                    <Label class="block text-lg font-semibold"> Audio {{ currentLangName }} </Label>
                    <audio
                        v-if="props.post.audio[currentLangCode].url"
                        :src="`/storage/${props.post.audio[currentLangCode].url}`"
                        controls
                        class="mt-2 w-full"
                    />
                    <div
